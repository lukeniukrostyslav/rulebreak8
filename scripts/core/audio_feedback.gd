...
func play_timeout() -> void:
    _play_tone(330.0, 0.07, 0.08)
    get_tree().create_timer(0.045).timeout.connect(_play_timeout_second_tone, CONNECT_ONE_SHOT)

func _play_timeout_second_tone() -> void:
    _play_tone(196.0, 0.09, 0.08)
...
func _push_tone(frequency: float, duration: float, amplitude: float) -> void:
    ...
    for i in frame_count:
        ...
        var envelope := minf(progress / 0.08, (1.0 - progress) / 0.18)
